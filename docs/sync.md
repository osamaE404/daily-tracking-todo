...
Known limits: single owner; no end-to-end encryption; the token remains in local browser storage until explicit disconnect; device storage can be removed by the browser or user; no push or scheduled notifications yet; dates retain their entered local wall time; pulling changes currently returns all records after the cursor without pagination. Export local backups before clearing storage. Server rollback behind a device cursor is rejected for manual recovery instead of silently discarding history.
